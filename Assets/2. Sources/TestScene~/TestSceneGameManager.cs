...
public class TestSceneGameManager : MonoBehaviour
{
 ...
 //       await _tcpClient.SendDataAsync(PROTO_MessageType.RequestSync, data);
	...
	async Awaitable OnTCPDataReceived(byte[] buffer, int length)
    {
	    ...
	    if (type == (int)SyncMessage_Type.RequestSync1)
        {
			M_BCSync_1 bcs
                = M_BCSync_1.Parser.ParseFrom(buffer, 12, length - 12);
            await Task.Run(() => { });
   ...
   //             _rcm.Synchronize = true;
    ...
    {
		SyncMessage_Type type = (SyncMessage_Type)BitConverter.ToInt32(buffer, 4);
        print($"OnUDPDataReceived: {length}");
		if (type == SyncMessage_Type.RequestSync1)
        {
